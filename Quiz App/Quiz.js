...
startQuizBtn.addEventListener('click', () => {
  ...
  shuffledQuestions = shuffleArray(questions).slice(0, 5);
  startQuiz();
  noOfQuestion();
  startTimer(30);
  timerLine(0);
...
nextQuesBtn.addEventListener('click', () => {
  if (quesCount < shuffledQuestions.length - 1) {
    quesCount++;
    quesIndex++;
    noOfQuestion();
    startQuiz();
...
let startQuiz = () => {
  let ques = `<h3>${quesIndex}. ${shuffledQuestions[quesIndex - 1].question}</h3>`;
  let quizOptions = `<div class="option option-1">
                 <div class="option-num-lett">A</div>
                 <span class="option-txt">${shuffledQuestions[quesIndex - 1].options[0]}</span>
               </div>
               <div class="option option-1">
                  <div class="option-num-lett">B</div>
                  <span class="option-txt">${shuffledQuestions[quesIndex - 1].options[1]}</span>
                </div>
                <div class="option option-1">
                  <div class="option-num-lett">C</div>
                  <span class="option-txt">${shuffledQuestions[quesIndex - 1].options[2]}</span>
                </div>
                <div class="option option-1">
                  <div class="option-num-lett">D</div>
                  <span class="option-txt">${shuffledQuestions[quesIndex - 1].options[3]}</span>
                </div>`;
...
let optionSelect = (userAns) => {
  ...
  const correctAns = shuffledQuestions[quesIndex - 1].answer;
...
//No Of Questions
let noOfQuestion = () => {
  noOfQues.innerHTML = `${quesIndex} of ${shuffledQuestions.length} Questions`;
}
...
let startTimer = (time) => {
  count = setInterval(() => {
    ...
    if (time == 0) {
      clearInterval(count)
      let allOptionTxt = optionBox.querySelectorAll('div span');
      let correctAns = shuffledQuestions[quesIndex - 1].answer;
      let AllOption = optionBox.children.length;
      for (let i = 0; i < allOptionTxt.length; i++) {
        if (allOptionTxt[i].textContent == correctAns) {
          allOptionTxt[i].parentElement.classList.add('correct');
...
let showResult = () => {
  ...
  if (score > 3) {
    let scoreTag = `<span>Congrats! You scored ` + score + ` out of ` + shuffledQuestions.length + ` 🔥</span>`;
    scoreText.innerHTML = scoreTag;
  } else if (score > 1) {
    let scoreTag = `<span>Nice! You scored ` + score + ` out of ` + shuffledQuestions.length + ` 🔥</span>`;
    scoreText.innerHTML = scoreTag;
  } else {
    let scoreTag = `<span>Sorry! You scored ` + score + ` out of ` + shuffledQuestions.length + ` 🔥</span>`;
    scoreText.innerHTML = scoreTag;
  }
}
